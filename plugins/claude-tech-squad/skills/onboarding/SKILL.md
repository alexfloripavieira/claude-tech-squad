...
Resolve the onboarding profile from the versioned catalog:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/bin/squad-cli onboarding-plan \
  --project-root . \
  --catalog plugins/claude-tech-squad/skills/onboarding/catalog.json
```

The returned plan selects:

- base template: `templates/claude-md/base.md`
- stack overlay: `templates/claude-md/stacks/{{detected_stack}}.md` or `templates/claude-md/stacks/generic.md`
- AI/LLM section: `templates/claude-md/ai-llm-section.md` when `ai_feature=true`
- specialists, recommended first command, and required health checks from `catalog.json`
...
Append the selected stack overlay from the catalog after the base template. If the detected stack has no explicit profile, use the `generic` profile and emit:

```
[Onboarding] No stack-specific CLAUDE.md overlay found — using generic profile.
```
