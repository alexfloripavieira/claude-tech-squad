...
- `mode=teammate`: tmux está disponível, `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`
  está ativo e a versão do Claude Code atende ao mínimo configurado.
- `mode=inline`: algum pré-requisito está ausente.

O modo `teammate` é preferido para visibilidade, panes separados e cross-talk
bloqueante. O modo `inline` é fallback válido: o workflow continua, mas a
...
- modo tmux como display preferido;
