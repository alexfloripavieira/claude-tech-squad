...
### `/claude-tech-squad:inception`

Use when you have a validated PRD and need the technical specification. Runs `inception-author` — produces `techspec.md` with architecture, risks, quality gates, and effort estimate (hours + days). Context7-first for every library referenced. Idempotent: reuses a valid existing TechSpec.

```text
/claude-tech-squad:inception <feature-slug>
```

Expects `ai-docs/prd-<slug>/prd.md` to exist (generated by `/discovery`). Standalone — does not chain into implementation. Chain with `/implement` to produce tasks and start building.
...
If Context7 is unavailable or does not have documentation for the library, the agent declares it explicitly and flags assumptions in the output. This applies to all 78 agents.
...
| Delivery Docs | prd-author, inception-author, tasks-planner, work-item-mapper |
