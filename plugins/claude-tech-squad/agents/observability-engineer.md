...
description: |
  Observability implementation specialist. PROACTIVELY use when adding structured logs, metrics, traces, alert rules, or ops dashboards to make systems diagnosable and on-call-friendly. Trigger on "OpenTelemetry", "tracing", "structured logs", "Prometheus", "Loki", or "observability". NOT for business/product analytics (use analytics-engineer) or release-governance reliability calls (use sre).<example>
