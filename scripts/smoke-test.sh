...
for skill in bug-fix cloud-debug dependency-check dashboard factory-retrospective from-ticket hotfix iac-review incident-postmortem llm-eval migration-plan multi-service onboarding pentest-deep pre-commit-lint prompt-review pr-review refactor release security-audit tech-debt-audit; do
  skill_file="$SKILLS_DIR/$skill/SKILL.md"
